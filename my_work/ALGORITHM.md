...
### 왜 그런가 — 저울과 시소만 있는 셈이기 때문

손목 센서가 정지 상태에서 읽을 수 있는 것은 딱 두 가지뿐입니다.

```
① 전체가 얼마나 무거운가      (힘)     = 저울과 같은 것
② 무게중심이 어디쯤인가       (비틀림)  = 시소의 균형점
```

**이게 전부입니다.** 그래서 **총무게와 무게중심을 둘 다 그대로 유지하는
재배치는 아무리 재도 구별되지 않습니다.**

```
   A 부위에서 100 g 을 덜어내고            무게중심이 원래 자리에
   B 부위에 100 g 을 얹었는데       →      그대로 있으면
                                          센서 값이 하나도 안 변한다
```

각도를 바꾸는 이유가 여기 있습니다. 관절을 접으면 부위들이 손목에서 얼마나
떨어져 있는지가 바뀌므로, **어떤 각도에서 숨어 있던 재배치가 다른 각도에서는
무게중심을 움직이게 됩니다.** 그때 비로소 보입니다.

> **자주 하는 오해**: "관절 축이 중력과 나란하면 안 보인다"고 생각하기 쉬운데,
> 이 코드처럼 **직교하는 세 방향으로 기울여 재면 중력 방향은 정보량에 아무
> 영향을 주지 않습니다.** 세 방향을 통째로 어떻게 돌려도 얻는 정보가 정확히
> 같기 때문입니다(15장의 "재미있는 사실"). 중력 방향이 문제가 되는 것은 한
> 방향으로만 잴 때입니다. 우리가 고르는 것은 오직 **관절 각도**입니다.
>
> 근거와 증명은 [THEORY.md](THEORY.md) 정리 1·2에 있습니다.
