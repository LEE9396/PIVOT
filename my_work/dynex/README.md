...
- 부피 7.45 cm³ 는 논문 표에서 온 값이고 저장소 안에 재현 경로가 없다(`MEASURED_HINGE_CM3`
  주석). 밀도 환산에만 쓰인다.
- 유지토크: 실측값이 없다. 코드 기본은 0.5 N·m (금속 HT-TC2626 0.5~0.6 범위), 안전계수 1.5
  → 예산 0.33 N·m. 플라스틱 HP-TC 는 0.15 N·m 로 3링크에 부족했다. **토크 게이지로 잰 뒤
  `HINGE_TORQUE` 에 넣을 것.** 실물 실험 코드(dual_view)는 기본값에서 힌지 검사를 안 한다.
