...
| 6 | **힌지 부피 실측** ← 새로 | 지금 코드의 7.45 cm³ 는 확인이 필요한 값입니다 (아래) |
...
커스텀 물체의 링크는 CAD 솔리드라 부피가 정의상 정확합니다. **힌지만 CAD 가
없어서 실측이 필요합니다.** 램프 링크에 쓴 것과 같은 절차입니다.

```
V = (물에 잠갔을 때 저울 증가량 [g]) / 0.997     -> cm^3   (실온 25 °C)
```

1. 비커에 물을 담아 저울에 올리고 **영점**.
2. 힌지를 실에 매달아 **바닥·벽에 닿지 않게** 완전히 잠급니다. 실은 가늘게.
3. 저울 증가량을 읽습니다. 그 g 수가 곧 밀려난 물의 질량입니다.
4. **힌지 2개를 한꺼번에** 재고 반으로 나누세요. 신호가 2배가 됩니다.

주의할 것 둘.

- **마찰 배럴에 공기가 갇힙니다.** 잠근 채로 몇 번 여닫고 톡톡 쳐서 기포를
  빼세요. 안 빼면 부피가 부풀어 나옵니다.
- **날개를 벌린 상태로** 재세요. 접으면 사이에 물이 안 들어가 그 틈이
  부피로 잡힙니다.

0.01 g 저울이면 7.45 cm³ 짜리를 0.15 % 로 잽니다. 0.1 g 저울이어도 2개를
같이 재면 0.7 % 라 충분합니다.

**재고 나면 고칠 곳은 한 줄입니다.**

```python
# density_id_objects.py
MEASURED_HINGE_CM3 = 7.45      # <- 실측값으로
```

**부위 질량 추정치는 이 값을 바꿔도 안 변합니다** (부피 불변성 — 밀도와
부피는 항상 곱으로만 들어갑니다). 바뀌는 것은 표에 싣는 힌지 밀도와, 그
밀도가 실측인지 가정인지의 격입니다. 지금 값 7.45 cm³ 는 41 g 과 합쳐
5503 kg/m³ 을 뜻하는데, 이건 플라스틱이 아니라 금속 값입니다 — 재보고 나면
`HINGES` 의 "PA6" 설명도 같이 맞춰야 할 수 있습니다.
