...
for name in PIVOT_ROOT MESHPCA_ROOT FOUNDATIONPOSE_ROOT SAM3_ROOT \
            ...
            FP_INIT_RGB FP_INIT_DEPTH FP_INTRINSICS FP_MASKS FP_OUTPUT; do
...
# --- 파지점 ---
if [[ -f "${GRASP_JSON}" ]]; then
  ok "파지점 ($(basename "${GRASP_JSON}"))"
else
  warn "파지점이 없습니다 — 지금 만듭니다"
  mkdir -p "${WORK}/outputs"
  ( cd "${WORK}" && "${R}" python grasp_target.py \
      --mesh "${FP_MESH_DIR}/${FP_GRASP_PART}_metric_watertight.ply" \
      --part "${FP_GRASP_PART}" --out "${GRASP_JSON}" ) || bad "파지점 생성" ""
fi
